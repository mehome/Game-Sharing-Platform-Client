...
BEGIN_MESSAGE_MAP(CTransparentListBox, CEdit)
...
int CTransparentListBox::AddString(CString Text,DWORD ItemData,BOOL Enabled)
{
   ...
   int Index = CEdit::AddString(Text);
   CEdit::SetItemData(Index,ItemData);
   return Index;
}
...
int CTransparentListBox::InsertString(int Index,CString Text, DWORD ItemData,BOOL Enabled)
{
   ...
   Index = CEdit::InsertString(Index,Text);
   CEdit::SetItemData(Index,ItemData);
   return Index;
}
...
void CTransparentListBox::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
   ...
   CFont *oldFont = DC->SelectObject(GetFont());
   DC->DrawText(NULL,&Rect,DT_CALCRECT|DT_EXPANDTABS|DT_NOPREFIX);
...
{
   SetRedraw(FALSE);
   int Ret = CEdit::SetTopIndex(Index);
   SetRedraw(TRUE);
   RedrawWindow(0,0,RDW_FRAME|RDW_INVALIDATE|RDW_UPDATENOW);
   return Ret;
}
